...
class Laporan extends CI_Controller {
	...
	public function index()
		{
			...
			$this->load->view('from_pencarian',$data);
		...
		public function cetak()
			{
				...
				$this->cek_menu($this->session->userdata('level'));
				// $this->load->view('content',$data);
				$this->load->view('from_cetak',$data);
	...
	function data(){
			 $data=$this->m_laporan->ambil_semua('t_lapor');
	 ...
	 function ambilid($id)
		{
			 ...
			 $data = $this->m_laporan->ambil_databyid($id);
			 echo json_encode($data);
		}
...
public function tambah()
{
	...
	$this->load->view('page_dashboard',$data);
	$this->cek_menu($this->session->userdata('level'));
	$data['barcode'] = $this->m_laporan->kode_otomatis();
...
$kode_otomatis = $this->m_laporan->kode_otomatis();
...
$this->m_laporan->insert('t_lapor',$data);
	 ...
	 public function form_edit($id)
				 {
					 // echo $id;
					 $ambilid = $this->m_laporan->ambil_databyid($id);
					 // echo "<pre>";
					 // print_r($ambilid);
					 // echo "</pre>";
...
function proses_ubah() {
    ...
    // 15[keterangan] =>
		...
		$this->m_laporan->update('t_lapor',$data,$where);
			redirect(base_url('t_lapor/'), 'refresh');
				 }
...
function hapus($id){
						...
						$this->m_laporan->hapus('t_lapor',$data);
 						redirect(base_url('t_lapor/'), 'refresh');
					}

function data_kecamatan(){
			  			 $data=$this->m_laporan->ambil_kecamatan('m_alamat_kecamatan');
			  			 echo json_encode($data);
			  	 }

function data_desa($id){
					 			  			 $data=$this->m_laporan->ambil_desa('m_alamat_desa',$id);
					 			  			 echo json_encode($data);
					 			  	 }

function data_kelompok(){
			 	$data=$this->m_laporan->ambil_kelompok('m_kelompok');
			  echo json_encode($data);
				 }

 function data_komoditas($id){
				 			 $data=$this->m_laporan->ambil_komoditas('m_komoditas',$id);
				 			 echo json_encode($data);
				 			 }


function data_kelas_kelompok(){
					 $data=$this->m_laporan->ambil_kelas_kelompok('m_kelas_kelompok');
				   echo json_encode($data);
							 			  	 }
...
public function proses_cari()
{
	...
	$where_kelompok = "AND t_lapor.id_kelompok=".$id_kelompok;
	}

	if ($id_kecamatan == 0) {
		$where_kecamatan = "";
	} else {
		$where_kecamatan = "AND t_lapor.id_kecamatan=".$id_kecamatan;
	}
	...
	if ($id_desa == 0) {
		...
		$where_desa = "AND t_lapor.id_desa=".$id_desa;
	}



$query='SELECT
id_lapor, t_lapor.nama_kelompok as nama_kelompok,
...
t_lapor.id_kelompok kelas_kelompok, m_kelompok.nama_kelompok as nama_kelompok_komoditas,
...
LEFT JOIN m_kelas_kelompok ON m_kelas_kelompok.id_kelas_kelompok = t_lapor.id_kelas_kelompok
WHERE tanggal_input >= '.$tanggal_awal.' AND  tanggal_input <= '.$tanggal_akhir.'  '.$where_kelompok.'  '.$where_kecamatan.'  '.$where_desa.'
ORDER BY id_kecamatan';
$data['file']=$this->m_laporan->ambil_cari($query);
$data['judul']="Pencarian Data";
$data['nama_user'] = $this->session->userdata('nama');
$this->load->view('page_dashboard',$data);
$this->cek_menu($this->session->userdata('level'));
...
t_lapor.id_komoditas,nama_komoditas,
t_lapor.id_kelas_kelompok, nama_kelas_kelompok,
nama_ketua,sk_pengukuhan,sk_kemenkuham, jumlah_anggota,
t_lapor.id_kelompok kelas_kelompok, m_kelompok.nama_kelompok as nama_kelompok_komoditas,
no_hp,status,no_hp, barcode
FROM t_lapor
LEFT JOIN m_alamat_kecamatan ON m_alamat_kecamatan.id_kecamatan = t_lapor.id_kecamatan
...
ORDER BY id_kecamatan';
$data['file']=$this->m_laporan->ambil_cari($query);
$data['tanggal_awal'] = $_POST['tanggal_awal'];
$data['tanggal_akhir'] = $_POST['tanggal_akhir'];
// $data['judul']="Pencarian Data";
// $data['nama_user'] = $this->session->userdata('nama');
// $this->load->view('page_dashboard',$data);
// $this->cek_menu($this->session->userdata('level'));
